# =========================================================================
# Conventional ConvNet with sigmoid non-linearity for MNIST classification
...
if __name__ == '__main__':
    ...
    for e in range(epochs):
        ...
        torch.save({'epoch': e,
                    ...
                    'loss': loss}, "./data/sigmoid_mnist_chk"+str(e)+".pt")
        if e % 2 == 0:
            er_.append(error)
            ac_.append(accur)
            print("Epoch: %d, Loss: %f, Accuracy: %f" % (e, rloss, accur))
    torch.save(net, "./data/mnist_det_sigmoid.pt")
    import sys
    num = sys.argv[1]
    np.save("./data/sigmoid_error_mnist_"+str(num)+"_det", er_)
    np.save("./data/sigmoid_accur_mnist_"+str(num)+"_det", ac_)
