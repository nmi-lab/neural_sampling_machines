# =========================================================================
# Conventional stochastic convnet (noisy rectifier) for MNIST classification
...
class noisy_rect(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        ctx.save_for_backward(x)
        z = torch.zeros_like(x).normal_(0, .1)
        return torch.nn.functional.relu(x + z)
...
if __name__ == '__main__':
    ...
    torch.save(net, "mnist_snn.pt")
    import sys
    num = sys.argv[1]
    np.save("./data/snn_error_mnist_norm_"+str(num), er_)
    np.save("./data/snn_accur_mnist_norm_"+str(num), ac_)
